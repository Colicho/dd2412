...
class MRI(H_functions):
    ...
    def __init__(self, channels, img_dim, usamp_factor, device):
        self.channels = channels
        self.img_dim = img_dim
        self.usamp_factor = usamp_factor
        # singular values are ones
        self._singulars = torch.ones(channels * img_dim**2, device=device)
    ...
    def U(self, vec):
        # we use the trivial solution for the SVD as USVt where U and S are diag(ones(...)).
        return vec.clone().reshape(vec.shape[0], -1)

    def Ut(self, vec):
        # U = Ut for U = diag(ones(...))
        return vec.clone().reshape(vec.shape[0], -1)
